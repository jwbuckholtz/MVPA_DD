...
class MemoryMonitor:
    ...
    def check_memory_threshold(self, required_gb: float) -> bool:
        ...
        available_with_buffer = memory_info['available_gb'] * (1 - self.config.AVAILABLE_MEMORY_BUFFER)
    ...
    def suggest_memmap(self, data_size_gb: float) -> bool:
        ...
        return (data_size_gb > self.config.MEMMAP_THRESHOLD_GB or 
                not self.check_memory_threshold(data_size_gb))
    ...
    def log_memory_usage(self, operation: str = ""):
        ...
        memory_info = self.get_memory_info()
        self.logger.info(f"Memory usage {operation}: {memory_info['process_memory_gb']:.2f} GB "
...
class MemoryMappedArray:
    ...
    def __init__(self, file_path: Union[str, Path], shape: Tuple[int, ...], 
                 dtype: np.dtype, mode: str = 'r', 
                 temp_file: bool = False, metadata: Dict[str, Any] = None):
        """
...
class MemoryEfficientLoader:
    ...
    def __init__(self, config: OAKConfig = None, memory_config: MemoryConfig = None):
        ...
        self.logger.info(f"Memory-efficient loader initialized. Temp dir: {self.temp_dir}")
    ...
    def create_memmap_from_nifti(self, nifti_path: Union[str, Path], 
                                subject_id: str = None) -> MemoryMappedArray:
        """
        ...
        if subject_id:
            ...
            filename = f"fmri_{hashlib.md5(str(nifti_path).encode()).hexdigest()[:8]}.dat"
        ...
        if memmap_path.exists():
            ...
            memmap_array = MemoryMappedArray(
                memmap_path, shape, dtype, mode='r', temp_file=True,
            ...
            memmap_array = MemoryMappedArray(
                memmap_path, shape, dtype, mode='w+', temp_file=True,
                metadata={'source': str(nifti_path), 'subject_id': subject_id}
            )
    ...
    def _copy_nifti_to_memmap(self, nifti_img: nib.Nifti1Image, 
                             memmap_array: MemoryMappedArray, 
                             chunk_size: int = 1000000):  # 1M voxels per chunk
    ...
    def load_fmri_memmap(self, subject_id: str, 
                        force_memmap: bool = False) -> Union[MemoryMappedArray, np.ndarray]:
        """
        ...
        use_memmap = (force_memmap or 
                     self.memory_config.USE_MEMMAP and 
                     self.monitor.suggest_memmap(size_gb))
        
        if use_memmap:
            self.logger.info(f"Loading {subject_id} using memory mapping ({size_gb:.2f} GB)")
    ...
    def extract_roi_timeseries_memmap(self, subject_id: str, roi_mask: np.ndarray,
    ...
    def create_shared_memmap(self, data: np.ndarray, identifier: str) -> MemoryMappedArray:
        ...
        # Create memory map
        memmap_array = MemoryMappedArray(
            memmap_path, data.shape, data.dtype, mode='w+', temp_file=True,
        ...
        memmap_array.close()
        memmap_array = MemoryMappedArray(
            memmap_path, data.shape, data.dtype, mode='r', temp_file=True,
            metadata={'identifier': identifier, 'shared': True}
        )
        
        self.active_memmaps.append(memmap_array)
        
        self.logger.info(f"Created shared memory map: {identifier} ({memmap_array.size_gb:.2f} GB)")
...
def create_memory_efficient_loader(config: OAKConfig = None, 
                                  memory_config: MemoryConfig = None) -> MemoryEfficientLoader:
...
class MemoryEfficientContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        memory_change = final_memory['process_memory_gb'] - self.initial_memory['process_memory_gb']
